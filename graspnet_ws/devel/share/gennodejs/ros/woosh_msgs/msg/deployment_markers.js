...
let deployment_marker = require('./deployment_marker.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class deployment_markers {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.markers.forEach((val) => {
      bufferOffset = deployment_marker.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new deployment_markers(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.markers[i] = deployment_marker.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    length += 62 * object.markers.length;
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new deployment_markers(null);
    ...
    if (msg.markers !== undefined) {
      ...
      for (let i = 0; i < resolved.markers.length; ++i) {
        resolved.markers[i] = deployment_marker.Resolve(msg.markers[i]);
      }
    }
    else {
      resolved.markers = []
...
module.exports = deployment_markers;
